...
function importPic() {
  var texture = new THREE.TextureLoader().load('/static/google.jpg', () => {
    // 平面
    const geometry = new THREE.PlaneGeometry(19 - 4, 10 - 4)
    const material = new THREE.MeshBasicMaterial({ map: texture })
    const plane = new THREE.Mesh(geometry, material)
    plane.position.set(-28, 0, -30)
    plane.rotation.set(Math.PI / 2, Math.PI, (6 * Math.PI) / 4)
    store.commit('addToScene', plane)
  })
}
